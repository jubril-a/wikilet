...
import PropertyForm from "@/src/features/property/components/PropertyForm"
...
function Step({label, goTo, active}: {label: string, active: boolean, goTo: () => void}) {
    return (
        <button className={cn("text-left text-sm p-2 pl-3 truncate", active && "border-l-4 border-l-primary-2 text-primary-2 bg-gray-100")} onClick={goTo}>{label}</button>  
    )
}
    ...
    return (
        <div className="">
            ...
            <div className="flex min-[440px]:bg-white">
                <nav className="hidden sm:grid gap-1 w-fit h-fit p-4 min-[480px]:px-8 min-[480px]:py-12">
                    {stepsLabel.map((label, index) => (
                        <Step key={label} label={label} active={index == currentStep} goTo={() => setCurrentStep(index)} />
                    ))}   
                </nav>
                {/* <p className="text-2xl font-semibold tracking-tight">{currentStep + 1}</p> */}
